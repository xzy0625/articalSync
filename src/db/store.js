...
export default class Store {
  ...
  getAccounts() {
    var accounts = chrome.storage.local.getItem('accounts')
    if (accounts == null) {
      accounts = []
    } else {
  ...
  getList(key) {
    var accounts = chrome.storage.local.getItem(key)
    if (accounts == null) {
      accounts = []
    } else {
      accounts = JSON.parse(accounts)
  ...
  addAccount({ uid, type, params, title }) {
    ...
    chrome.storage.local.setItem('accounts', JSON.stringify(accounts))
    return true
  }
  ...
  editTask(tid, obj) {
    ...
    chrome.storage.local.setItem('tasks', JSON.stringify(tasks))
