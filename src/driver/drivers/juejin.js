import { modifyRequestHeaders } from "../../utils";
import Adapter from "./superDriver";

export default class JuejinAdapter extends Adapter {
  constructor() {
    super("0.0.1", "掘金", {
  ...
  async getMetaData() {
    var data = await this.innerRequest({
  ...
  modifyRequestHeaders() {
    return modifyRequestHeaders('api.juejin.cn', {
      Origin: 'https://juejin.cn',
      Referer: 'https://juejin.cn/'
    }, [
      '*://api.juejin.cn/*',
  ...
  async preEditPost(post) {
    ...
    try {
      ...
      post.content = tempDoc.children("div").length == 1 ? tempDoc.children("div").html() : tempDoc.html();
